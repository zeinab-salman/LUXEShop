...
export default function OrdersDashboard() {
  ...
  useEffect(() => {
    try {
      ...
      const completedData = JSON.parse(localStorage.getItem("completed-orders")) || [];
  ...
  const handleOrderCompleted = useCallback((orderId) => {
    const orderToComplete = orders.find(order => order.userId.id === orderId);

    if (orderToComplete) {
      const updatedOrders = orders.filter(order => order.userId.id !== orderId);
      ...
      localStorage.setItem("completed-orders", JSON.stringify(updatedCompletedOrders));
    ...
    <section className="orders-dash-section flex-center">
      ...
      {orders.length > 0 ? (
        orders.map((order, index) => {
          ...
          const { name = "Unknown", category = "N/A", img = "placeholder.png" } = firstItem;
