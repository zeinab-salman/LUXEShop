...
export default function VerificationCodeModel() {
    ...
    const validateCode = () => {
        ...
        const storedCode = JSON.parse(localStorage.getItem("verificationCode"))?.[0]?.code || '';
    ...
    const handleSubmit = (event) => {
        ...
        const allFieldsFilled = Object.values(formData).every(code => code !== '');
        if (!allFieldsFilled) {
    ...
    const handleResendCode = (event) => {
        ...
        localStorage.setItem("verificationCode", JSON.stringify([{ code: "123456" }]));
    };
    ...
    return (
        <section className="verification-section">
            <Title title="Sign in with two-step verification " type="sections-description" line="line-sec" />

            <p id="send-note">we've sent a<span> 6</span> digit code to your email</p>
